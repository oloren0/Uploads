...
def print_performance_metrics(predictions):
    ...
    print("AUC (Area Under ROC) = {:.3f}".format(auc))
    print("AUPR (Area Under PR) = {:.3f}".format(aupr))

    # Get RDD of (prediction, label) tuples for mllib evaluation
    ...
    print("  Confusion Matrix:\n{}".format(multi_metrics.confusionMatrix().toArray()))

    # Iterate over classes for detailed metrics
    ...
    for label in labels:
        ...
        print("  F1 Score = {:.3f}".format(multi_metrics.fMeasure(label)))
        print("  False Positive Rate = {:.3f}".format(multi_metrics.falsePositiveRate(label)))
...
# Access the model summary and coefficients
coefficients = lrModel.coefficients
intercept = lrModel.intercept

# Extract feature names from the 'features' column's metadata
feature_names = [x["name"] for x in sorted(
    trainingData.schema["features"].metadata["ml_attr"]["attrs"].get("binary", []) +
    trainingData.schema["features"].metadata["ml_attr"]["attrs"].get("numeric", []),
    key=lambda x: x["idx"]
)]

# Combine feature names and coefficients
feature_coefficients = [(feature_names[i], coefficients[i]) for i in range(len(coefficients))]

from pyspark.sql.types import StructType, StructField, StringType, DoubleType

# Ensure coefficients are converted to Python floats
feature_coefficients = [(feature, float(coeff)) for feature, coeff in feature_coefficients]
...
df = spark.createDataFrame(feature_coefficients, schema=schema)

# Sort the DataFrame by feature names and coefficients
df_sorted = df.orderBy(["Feature", "Coefficient"])
...
feature_names = [
    attr["name"]
    for attr in chain(*trainingData.schema["features"].metadata["ml_attr"]["attrs"].values())
]

# Combine feature names with their importance scores
feature_importance = list(zip(feature_names, rfFeatureImportance.toArray()))
