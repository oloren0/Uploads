...
--Tennis courts one and two, massage rooms one and two, and the squash court.
...
--Four facilities: Badminton, table tennis, snooker table, and the pool table.
...
SELECT name, monthlymaintenance, new_column
FROM Facilities
WHERE 
	CASE WHEN monthlymaintenance > 100 THEN 'expensive'
    ELSE 'cheap' END AS new_column
...
SELECT 
	f.name AS facility
    CASE WHERE b.memid = 0 THEN 'guest' = 1 AS guest
    	ELSE 'member' = 1 AS member END 
    SUM(f.membercost*COUNT(guest) + f.guestcost*COUNT(guest) AS total_revenue 
FROM Facilities as f 
LEFT JOIN Bookings as b 
ON b.facid = f.facid
